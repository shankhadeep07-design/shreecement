...
export default function BudgetTransferKanbanSendForApproval({ permissions = [] }) {
  const [budgets, setBudgets] = useState([]);
  const [loading, setLoading] = useState(false);
  const [loadMoreLoading, setLoadMoreLoading] = useState(false); // 🔹 new state for button spinner
  const [pagination, setPagination] = useState({ current: 1, pageSize: 3, total: 0 });
  const [filtersVisible, setFiltersVisible] = useState(false);
  const [showModal, setModalStatus] = useState({ user_update_modal: false });
  ...
  const fetchBudgets = async (page = 1, append = false) => {
    try {
      if (append) {
        setLoadMoreLoading(true); // show button spinner
      } else {
        setLoading(true); // show first load spinner
      }

      const token = getAuthToken();
      let query = `status=send_for_approval&page=${page}&limit=${pagination.pageSize}`;
  ...
  return (
    <div className="kanban-column">
      {/* 🔹 Header */}
      <div className="kanban-header header-col-4" >
        <span >
        ...
        <span className="kanban-count">
          <Tag color="#8e0ed8ff">{pagination.total}</Tag>
        </span>
      ...
      {loading && pagination.current === 1 ? (
        ...
        <>
          {budgets.map((budget) => (
            <Card key={budget.tbm_id} size="small" className="budget-card"  style={{ borderBottom: "solid 4px #c475d8ff" }}>
              {/* 🔹 Top Row: FY + Amount */}
              <div className="card-top-row">
                ...
                <div className="card-info">
                  ...
                  <span className="card-text-bold">₹{budget.tbm_total_budget_amount}</span>
                </div>
              </div>
              ...
              <div className="card-approvers">
                ...
                >
                  <div className="approvers-avatars" style={{width:'28px'}}>
                    {budget?.approvers?.length > 0 ? (
                      budget.approvers.map((approver, i) => (
...
// ✅ Approval Tracking Sub-component
    ...
    <div className="approval-track-list">
      {visibleData.map((track) => (
        <div key={track.apt_id} className="approval-track-item">
          <div className="approver-circle me-2">
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ marginTop: 6, fontSize: 12 }}>
              📝 Step: <strong>{track.apt_accept_step || "N/A"}</strong> | ✅ Status:{" "}
