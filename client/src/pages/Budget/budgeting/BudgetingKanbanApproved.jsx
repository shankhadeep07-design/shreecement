...
import BudgetAmmendmentModal from "./BudgetAmmendmentModal.jsx";
...
export default function BudgetingKanbanApproved({ permissions = [] }) {
  ...
  const openProjectModal = async (budgetId) => {
    try {
      const { data } = await budgetingDetailsApi({ budgeting_id: budgetId });

      setProjectBudgetList(data?.tbm_budget_list || []);
      setProjectModal(true);
    } catch (error) {
      toast.error("Failed to load budget allocations");
    }
  };
  const openAmendmentModal = (budgetId) => {
  ...
  const projectColumns = [
    ...
    {
      title: "Thematic Area (Schedule VII Item No.)",
    ...
    {
      title: "Proposed Budget (INR)",
      dataIndex: "tbad_amount",
      render: (amt) => `₹ ${amt?.toLocaleString() || 0}`,
    },

    // ✅ Independent + icon for each row
    {
      title: "Action",
      key: "action",
      render: (_, record) => (
        <PlusCircleOutlined
          ...
          onClick={() =>
            window.open(
              `${import.meta.env.VITE_HOME_PAGE}/admin/project/create/${record.tbad_id}`,
              "_blank"
            )
          }
        />
    ...
    <>
      <div className="kanban-column">
        ...
        <div className="kanban-budget-list-box">
            ...
            <>
              {budgets.map((budget) => (
                <Card
                  ...
                  style={{ borderBottom: "solid 4px #a7c816" }}
                >
                  {/* 🔹 Top Row: FY + Amount */}
                  ...
                  <div className="card-approvers">
                    <Popover
                      ...
                      title={
                        ...
                        <div className="popover-title"> <span className="icon-Approver"><FaUsers /> </span> Approver Details</div>
                      }
                      content={
                  ...
                  <Popover
                    ...
                    placement="top"
                    title={
                      <div className="popover-title"><span className="icon-Approver"><FaUsers /> </span>  Approval Tracking</div>
                    }
                  ...
                  <div className="card-actions">
                    ...
                    <Tooltip title="Create Project">
                      <Button
                        ...
                        size="small"
                        icon={<PlusOutlined />}
                        onClick={() => openProjectModal(budget.tbm_id)}
                      >
                        Create Project
                      </Button>
                    </Tooltip>
          ...
          {showModal && (
            <BudgetAmmendmentModal
              showModal={showModal}
              budgetId={selectedBudgetId} // 👈 pass budget id
      ...
      {approversData.length > 5 && (
        <div style={{ textAlign: "center", marginTop: 8 }}>
          <a onClick={() => setExpanded(!expanded)}>
            {expanded
              ? "Show Less ▲"
              : `Show More (${approversData.length - 5}) ▼`}
          </a>
        </div>
      )}
    </div>
