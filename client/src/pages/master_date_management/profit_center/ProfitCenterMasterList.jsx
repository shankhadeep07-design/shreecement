...
import { getExcelExportFactoryList } from '../../../services/MasterData-service.js';
import { AddEditProfitCenter } from './AddEditProfitCenter.jsx';

import { statusChange } from '../../../Services/Master-service.js';
const ProfitCenterMasterList = () => {

    const [showModal, setShowModal] = useState(false);
    let [editProfitCenter, setEditProfitCenter] = useState("#ffffff");
    let datatable_url = `${import.meta.env.VITE_API_URL}/admin/masters-management/profit-center-list/datatable`;
    const [permissions, setPermissions] = useState([])
    const [exportsLists, setExportsLists] = useState([]);
    useEffect(() => {
        getMyModulePermissionFun('profit_center')
            .then((module) => {
  ...
  const handleStatusChange = async (pk, newStatus) => {
    ...
    const initiatedProfitCenterDatatable = () => {
        var i = 1;
        var authToken = getAuthToken();
        var my_url = `${import.meta.env.VITE_API_URL}/admin/masters-management/profit-center-list/datatable`;

        if ($.fn.DataTable.isDataTable("#initiated-ProfitCenter-datatable")) {
            $("#initiated-ProfitCenter-datatable").DataTable().destroy();
        }
        $("#initiated-ProfitCenter-datatable").DataTable({
            order: [[1, "asc"]],
            ...
            columns: [
                ...
                },
                {
                 ...
                 {
                    name: "tprofc_is_active",
                    data: "tprofc_is_active",
                    searchable: false,
                    orderable: false,
                ...
                <div class="form-check form-switch d-inline-block"
                    ...
                    <input class="form-check-input toggle-switch"
                           type="checkbox"
                           id="toggleSwitch_${row.tprofc_id}"
                           data-id="${row.tprofc_id}"
                          
                           ${isChecked}  data-table="t_profit_center_master"   >
                </div>
            `;
                    }
                },
    ...
    useEffect(() => {
        if (permissions.indexOf('list') > -1) {
            initiatedProfitCenterDatatable();
        }

    }, [permissions]);

    const getAllExportData = () => {
        getExcelExportFactoryList().then((response) => {
    ...
    useEffect(() => {
        if (exportsLists.length > 0) {
            tableToExcel("new-table", "Profit Center Master List"); // Trigger export once data is set
        }
    }, [exportsLists]);
            ...
            <div className="home-content">
                <div className="card pb-3">
                        ...
                        <div className="float-right">
                            {
                                ...
                                <button
                                    ...
                                    onClick={() => addFun()}
                                >
                                    <i class="fa-solid fa-plus"></i> Add Profit Center
                    ...
                    <div className="card-body at-elevation-z6 table-box" style={{ maxHeight: "calc(100vh - 23vh);", overflowX: 'auto' }}>
                        ...
                        <div className="initiated-ProfitCenter-table-container">
                            <div className="mt-2 table table-bordered">
                                <div>
                                    {(permissions.indexOf('list') > -1) ?
                                        <table id="initiated-ProfitCenter-datatable" className="table table-bordered dataTable">
                                            <thead>
                                                <tr>
                                                    <th>ID</th>
                                                    <th>Corporate</th>
                ...
                <div className="allModals">
                    ...
                    >
                        <Modal.Header closeButton>
                            <Modal.Title>{editProfitCenter == '' ? 'Add Profit Center' : 'Update Profit Center'}</Modal.Title>
                        </Modal.Header>

                        <Modal.Body>
                            <AddEditProfitCenter
                                changeModalStatus={changeModalStatus}
                                editProfitCenter={editProfitCenter}
                                initiatedProfitCenterDatatable={initiatedProfitCenterDatatable}
                    ...
                    <tbody>
                        {exportsLists?.map((data, index) => (
                            <tr key={index}>
                                ...
                                <td>{data?.tloc_location_name || ""}</td>
                                <td>{data?.tfact_factory_name || ""}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
...
export default ProfitCenterMasterList;
