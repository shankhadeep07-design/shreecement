...
const SDGMappingMaster = () => {
    let { loading, setLoading } = useLoading(false);
    const [showModal, setShowModal] = useState(false);
    let [editSdg, setEditSdg] = useState("#ffffff");
    let datatable_url = `${import.meta.env.VITE_API_URL}/admin/priority-alignment/sdg-master-list/datatable`;
    ...
    const initiatedSdgDatatable = () => {
        ...
        var my_url = `${import.meta.env.VITE_API_URL}/admin/priority-alignment/sdg-master-list/datatable`;
            ...
            columns: [
                ...
                <div class="form-check form-switch d-inline-block"
                    ...
                    <input class="form-check-input toggle-switch"
                           type="checkbox"
                           id="toggleSwitch_${row.tsdg_id}"
                           data-id="${row.tsdg_id}"
                          
                           ${isChecked}  data-table="t_sdgs"   >
                </div>
            `;
                    }
                },
    ...
    const getAllExportData = () => {
        getExcelExportSdgList().then((response) => {
    ...
    useEffect(() => {
        if (exportsLists.length > 0) {
            tableToExcel("new-table", "SDGsList"); // Trigger export once data is set
        }
    }, [exportsLists]);
                    ...
                    <div className="card-header fw-bold d-flex justify-content-between align-items-center py-2 px-3">
                        ...
                        <div className="float-right">
                            <button
                                ...
                                onClick={() => addFun()}
                            >
                                <i class="fa-solid fa-plus"></i> Add SDG
                ...
                >
                    ...
                    <tbody>
                        {exportsLists?.map((data, index) => (
                            <tr key={index}>
                                <td>{index + 1}</td>
                                <td>{data?.tsdg_name || ""}</td>
                                <td>{data?.tsdg_desc || ""}</td>
...
export default SDGMappingMaster;
