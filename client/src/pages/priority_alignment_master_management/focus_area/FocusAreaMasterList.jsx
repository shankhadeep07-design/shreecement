...
import { AddEditFocusArea } from './AddEditFocusArea.jsx';

const FocusAreaMasterList = () => {
    let { loading, setLoading } = useLoading(false);
    const [showModal, setShowModal] = useState(false);
    let [editFocusArea, setEditFocusArea] = useState("#ffffff");
    let datatable_url = `${import.meta.env.VITE_API_URL}/admin/priority-alignment/sub-schedule-seven-master-list/datatable`;
    const [permissions, setPermissions] = useState([])
    const [exportsLists, setExportsLists] = useState([]);
    useEffect(() => {
        getMyModulePermissionFun('focus_area_list')
            .then((module) => {
    ...
    const editFun = (data) => {
        setEditFocusArea(data);

        changeModalStatus("user_update_modal", true);
    };


    const initiatedFocusAreaDatatable = () => {
        var i = 1;
        var authToken = getAuthToken();

        var my_url = `${import.meta.env.VITE_API_URL}/admin/priority-alignment/sub-schedule-seven-master-list/datatable`;

        if ($.fn.DataTable.isDataTable("#initiated-FocusArea-datatable")) {
            $("#initiated-FocusArea-datatable").DataTable().destroy();
        }


        $("#initiated-FocusArea-datatable").DataTable({
            order: [[1, "asc"]],
    ...
    useEffect(() => {
        if (permissions.indexOf('list') > -1) {
            initiatedFocusAreaDatatable();
        }

    }, [permissions]);

    const getAllExportData = () => {
        getExcelExportSubScheduleSevenList().then((response) => {
    ...
    useEffect(() => {
        if (exportsLists.length > 0) {
            tableToExcel("new-table", "Sub Schedule List"); // Trigger export once data is set
        }
    }, [exportsLists]);
            ...
            <div className="home-content">
                <div className="card pb-3">
                        ...
                        <div className="float-right">
                            {
                                ...
                                <button
                                    ...
                                    onClick={() => addFun()}
                                >
                                    <i class="fa-solid fa-plus"></i> Add Sub Schedule
                    ...
                    <div className="card-body at-elevation-z6 table-box" style={{ maxHeight: "calc(100vh - 23vh);", overflowX: 'auto' }}>
                        ...
                        <div className="initiated-FocusArea-table-container">
                            <div className="mt-2 table table-bordered">
                                <div>
                                    {(permissions.indexOf('list') > -1) ?
                                        <table id="initiated-FocusArea-datatable" className="table table-bordered dataTable">
                                            <thead>
                                                <tr>
                                                    <th>ID</th>
                ...
                <div className="allModals">
                    ...
                    >
                        <Modal.Header closeButton>
                            <Modal.Title>{editFocusArea == '' ? 'Add Subschedule' : 'Update Subschedule'}</Modal.Title>
                        </Modal.Header>
                        ...
                        <Modal.Body>
                            <AddEditFocusArea
                                ...
                                initiatedFocusAreaDatatable={initiatedFocusAreaDatatable}
...
export default FocusAreaMasterList;
