...
import { createProject, projectDetailsApi } from "../../services/Project-service";
...
const Schema = Yup.object({
    ...
    tproj_block_id: Yup.array()
        .min(1, "At least one Block required"),

    tproj_gram_panchayat_id: Yup.array()
        .min(1, "At least one Gram Panchayat required"),

    tproj_revenue_village_id: Yup.array()
        .min(1, "At least one Revenue Village required"),

    tproj_village_id: Yup.array()
        .min(1, "At least one Village required"),

    // 🔹 THEME
    tproj_theme_id: Yup.mixed().required("Theme required"),
    tproj_schedule_id: Yup.mixed().required("Schedule required"),
    tproj_sub_schedule_id: Yup.mixed().required("Sub Schedule required"),
...
const ProjectAddEditForm = ({ closeModal, reloadTable, details }) => {
    ...
    // const [allGramPanchayats, setAllGramPanchayats] = useState([]);
    // const [allRevenueVillages, setAllRevenueVillages] = useState([]);
    ...
    useEffect(() => {
        if (formData.tproj_unit_id) {
            loadUnitLocation(formData.tproj_unit_id);
        }
    ...
    const loadUnitLocation = async (unitId) => {
        try {
            ...
            // ✅ Find correct state from existing list
            const selectedState = states.find(
                (s) => String(s.value) === String(data.state_id)
            )

            // setStates(selectedState ? [selectedState] : []);
            ...
            // setAllGramPanchayats(data.grampanchayats || []);
            // setAllRevenueVillages(data.revenue_villages || []);
            ...
            setFormData(prev => ({
                ...
                // tproj_block_id: data.blocks?.map(b => b.value) || [],
                // tproj_gram_panchayat_id: data.grampanchayats?.map(g => g.value) || [],
                // tproj_revenue_village_id: data.revenue_villages?.map(rv => rv.value) || [],
                // tproj_village_id: data.villages?.map(v => v.value) || [],
            }));
    ...
    useEffect(() => {
        if (formData.tproj_theme_id) {
            loadSchedules(formData.tproj_theme_id);
        } else {
            setSchedules([]);
            setSubSchedules([]);
            setFormData(prev => ({
    ...
    }, [formData.tproj_theme_id]);
    useEffect(() => {
        if (formData.tproj_schedule_id) {
            loadSubSchedules(formData.tproj_schedule_id);
        } else {
            setSubSchedules([]);
            setFormData(prev => ({
                ...prev,
    ...
    const loadSchedules = async (themeId) => {
        try {
            const res = await getScheduleSevenByThemeIdApi(themeId);

            // ⚠️ IMPORTANT: check structure
            const data = res?.data || [];
    ...
    useEffect(() => {
        if (formData.tproj_theme_id) {
            fetchKpiByThemeId(formData.tproj_theme_id).then((res) => {
                const list = res?.data || [];
    ...
    useEffect(() => {
        if (
            ...
            const payload = {
                ...
                tproj_theme_id: formData.tproj_theme_id,
                tproj_schedule_id: formData.tproj_schedule_id,
    ...
    useEffect(() => {
        if (details?.tproj_id) {
            projectDetailsApi({ tproj_id: details?.tproj_id })
                .then(({ data }) => {
    ...
    useEffect(() => {
        ...
        // SDG mapping
        const sdgIds = data.tproj_sdg_id?.map(s => s.sdg_id) || [];
        ...
        // KPI mapping
        // const kpiIds = data.kpis?.map(k => k.kpi_id) || [];
        const kpiIds = data.kpis || [];
        ...
        setFormData(prev => ({
            ...
            tproj_id: data.tproj_id, // 🔥 IMPORTANT FOR UPDATE

            tproj_fy_id: data.tproj_fy_id,
            tproj_unit_id: data.tproj_unit_id,
            tproj_state_id: data.tproj_state_id,
            ...
            tproj_theme_id: data.tproj_theme_id,
            ...
            tproj_project_started_necessarily: Number(data.tproj_project_started_necessarily),
            ...
            kpis: kpiIds.length ? kpiIds : [null]
        }));

    }, [projectDetails]);

    useEffect(() => {
        if (projectDetails?.tproj_theme_id) {
            loadSchedules(projectDetails.tproj_theme_id);
        }
    }, [projectDetails]);

    useEffect(() => {
        if (projectDetails?.tproj_schedule_id) {
            loadSubSchedules(projectDetails.tproj_schedule_id);
        }
    }, [projectDetails]);
    ...
    return (
        <Card>
            ...
            <Row gutter={[16, 16]}>
                    ...
                    <Select mode="multiple" style={{ width: "100%" }}
                        ...
                        onChange={async (selectedBlocks) => {
                            try {
                                ...
                                console.log(err);
                                message.error("Failed to load Gram Panchayat");
                            }
                        }}
                    />
                    ...
                    <Select mode="multiple" style={{ width: "100%" }}
                        ...
                        onChange={async (selectedGPs) => {
                            try {
                                ...
                                console.log(err);
                                message.error("Failed to load Revenue Villages");
                            }
                        }}
                    />
                ...
                <Col span={12}>
                    <label>Revenue Village <span style={{ color: "red" }}>*</span></label>
                    <Select mode="multiple" style={{ width: "100%" }}
                ...
                <Col span={12}>
                    ...
                    <Select style={{ width: "100%" }}
                        options={themes}
                        value={formData.tproj_theme_id}
                        onChange={(v) => handleChange("tproj_theme_id", v)} />
                    {errors?.tproj_theme_id && (
                        <div className="text-danger">{errors.tproj_theme_id}</div>
                    )}
                </Col>
                ...
                <Col span={12}>
                    ...
                    <Select style={{ width: "100%" }}
                        options={schedules}
                        value={formData.tproj_schedule_id}
                        onChange={(v) => handleChange("tproj_schedule_id", v)} />
                ...
                <Col span={12}>
                    <label>Sub Schedule <span style={{ color: "red" }}>*</span></label>
                    <Select style={{ width: "100%" }}
                        options={subSchedules}
                        value={formData.tproj_sub_schedule_id}
                        onChange={(v) => handleChange("tproj_sub_schedule_id", v)} />
                ...
                <Col span={12}>
                    ...
                    <Select
                        ...
                        onChange={(value) => {
                            ...
                            // ✅ remove unselected SDG weightages
                                ...
                                <Row key={sdgId} gutter={10} style={{ marginBottom: 8 }}>
                                    ...
                                    <Col span={12}>
                                        ...
                                        {errors?.sdgWeightages && (
                                            <div className="text-danger">{errors.sdgWeightages}</div>
                                        )}
                                    </Col>
                ...
                <Col span={12}>
                    ...
                    <Input style={{ width: "100%" }}
                        ...
                        onChange={(e) => handleChange("tproj_project_title", e.target.value)}
                    />
                ...
                <Col span={12}>
                    ...
                    <DatePicker style={{ width: "100%" }}
                        value={formData.tproj_project_start_date ? dayjs(formData.tproj_project_start_date) : null}
                        onChange={(d) => handleChange("tproj_project_start_date", d?.format("YYYY-MM-DD"))} />
                ...
                <Col span={12}>
                    ...
                    <DatePicker style={{ width: "100%" }}
                        value={formData.tproj_project_end_date ? dayjs(formData.tproj_project_end_date) : null}
                        disabledDate={(c) =>
                            c && c.isBefore(dayjs(formData.tproj_project_start_date))
                        }
                        onChange={(d) => handleChange("tproj_project_end_date", d?.format("YYYY-MM-DD"))} />
                ...
                {/* <Col span={12}>
                    ...
                    <InputNumber style={{ width: "100%" }}
                        ...
                        onChange={(v) => handleChange("tproj_budget_amount", v)} />
                </Col> */}
                <Col span={12}>
                    <label>Project Budget Amount <span style={{ color: "red" }}>*</span></label>
                    ...
                    <InputNumber
                        ...
                        //             tproj_budget_amount: "Budget cannot be negative"
                        //         }));
                        //     } else if (value > budgetInfo.remaining) {
                        //         setErrors(prev => ({
                        //             ...prev,
                        //             tproj_budget_amount: "Budget cannot exceed remaining amount"
                        //         }));
                        //     } else {
                        //         setErrors(prev => ({
                        //             ...prev,
                    ...
                    <div style={{ marginTop: 8 }}>
                        <div><b>Total Budget:</b> {budgetInfo.total}</div>
                        <div><b>Remaining Amount:</b> {budgetInfo.remaining}</div>
                        <div><b>Project Amount:</b> {budgetInfo.project}</div>
                    </div>
                </Col>
                <Col span={12}>
                ...
                <Col span={12}>
                    ...
                    {formData.kpis.map((kpi, index) => {
                        ...
                        const availableOptions = kpiList.filter(
                            (item) =>
                                !selectedKpis.includes(item.value) ||
                                item.value === kpi
                        );
            ...
            <Button type="primary" loading={loading} onClick={handleSubmit} style={{ marginTop: 20 }}>
                Submit
            </Button>
        </Card>
    );
