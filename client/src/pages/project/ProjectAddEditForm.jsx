...
const ProjectAddEditForm = ({ closeModal, reloadTable, details }) => {
  ...
  //////add and edit both case this method is called. both cases via handle change,and edit via useeffect to prepopulate data.
  const loadUnitLocation = async (unitId, approvedType, fyId) => {
    try {
      ...
      setBlocks(newBlocks);
      setGramPanchayats(newGPs);
      setRevenueVillages(newRVs);
      setVillages(newVillages);

      // ✅ FILTER OLD VALUES AGAINST NEW OPTIONS
      setFormData((prev) => ({
        ...prev,
        tproj_state_id: data.state_id,
        tproj_district_id: data.district_id,

        tproj_block_id: prev.tproj_block_id.filter((id) =>
          newBlocks.some((b) => String(b.value) === String(id)),
        ),

        tproj_gram_panchayat_id: prev.tproj_gram_panchayat_id.filter((id) =>
          newGPs.some((g) => String(g.value) === String(id)),
        ),

        tproj_revenue_village_id: prev.tproj_revenue_village_id.filter((id) =>
          newRVs.some((rv) => String(rv.value) === String(id)),
        ),

        tproj_village_id: prev.tproj_village_id.filter((id) =>
          newVillages.some((v) => String(v.value) === String(id)),
        ),
      }));
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
        if (formData.tproj_schedule_id) {
            fetchKpiByScheduleViiId(formData.tproj_schedule_id).then((res) => {
                const list = res?.data || [];
  ...
  const handleChange = useCallback((name, value) => {
    setFormData((prev) => {
      ...
      // 🔥 ADD HERE
      if (name === "tproj_approved_type") {
        updated.tproj_block_id = [];
        updated.tproj_gram_panchayat_id = [];
        updated.tproj_revenue_village_id = [];
        updated.tproj_village_id = [];
  ...
  // useEffect(() => {
  //   if (formData.tproj_approved_type === "under_approved_annual_budget") {
  //     setLastPayload(null); // ✅ ensures API triggers in edit mode
  //   }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      /* ✅ APPLY ONLY FOR APPROVED BUDGET */
      if (formData.tproj_approved_type === "under_approved_annual_budget") {
        const allowedBudget = budgetInfo.remaining;
      ...
      message.success("Project saved successfully");
          setLastPayload(null);  // ✅ reset here after submission
  ...
  // fetch budget is calling via useeffect both add and edit and on change ists check the last payload always
  ...
  useEffect(() => {
    ...
    if (
      ...
      const isSame =
        lastPayload && JSON.stringify(payload) === JSON.stringify(lastPayload);
  ...
  useEffect(() => {
    ...
    if (data.tproj_unit_id && data.tproj_approved_type) {
      loadUnitLocation(
        data.tproj_unit_id,
