...
const MasterReport = () => {
  ...
  const fetchData = async (page = 1, pageSize = 10) => {
    setLoading(true);
    try {
      const resp = await getMasterReport({
  ...
  const handleExport = async () => {
    setLoading(true);
    try {
      const resp = await exportMasterReport({
        start_date: filters.start_date,
        end_date: filters.end_date,
      });
      ...
      link.href = url;
      link.setAttribute("download", "MasterAuditReport.xlsx");
          ...
          (() => {
            ...
            return (
              <div>
                {/* ✅ Header Info */}
                {/* <p>
                  <strong>Changed By:</strong> {selectedLog.username || "-"}
                </p> */}
                <p>
