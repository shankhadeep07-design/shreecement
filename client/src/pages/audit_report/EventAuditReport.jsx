...
const EventAuditReport = () => {
  ...
  const fetchData = async (page = 1, pageSize = 10) => {
    setLoading(true);
    try {
      const resp = await getEventReport({
  ...
  const handleExport = async () => {
    setLoading(true);
    try {
      const resp = await exportEventReport({
        start_date: filters.start_date,
        end_date: filters.end_date,
      });
      ...
      link.href = url;
      link.setAttribute("download", "EventAuditReport.xlsx");
...
export default EventAuditReport;
