...
const BudgetAuditReport = () => {
  ...
  const fetchData = async (page = 1, pageSize = 10) => {
    setLoading(true);
    try {
      const resp = await getBudgetReport({
  ...
  const handleExport = async () => {
    setLoading(true);
    try {
      const resp = await exportBudgetReport({
        start_date: filters.start_date,
        end_date: filters.end_date,
      });
      ...
      link.href = url;
      link.setAttribute("download", "BudgetAuditReport.xlsx");
...
export default BudgetAuditReport;
