...
import {
  getProposalReport,
  exportProposalReport,
} from "../../services/Audit-report-service";

const { RangePicker } = DatePicker;

const ProposalAuditReport = () => {
  ...
  const fetchData = async (page = 1, pageSize = 10) => {
    setLoading(true);
    try {
      const resp = await getProposalReport({
  ...
  const handleExport = async () => {
    setLoading(true);
    try {
      const resp = await exportProposalReport({
        start_date: filters.start_date,
        end_date: filters.end_date,
      });
      ...
      link.href = url;
      link.setAttribute("download", "ProposalAuditReport.xlsx");
                    ...
                    {keys.map((key) => {
                      ...
                      return (
                        <tr
                          key={key}
                          style={{
                            background: isChanged ? "#ffeaea" : "transparent",
                          }}
                        >
                          <td>
                            <strong>{key}</strong>
...
export default ProposalAuditReport;
