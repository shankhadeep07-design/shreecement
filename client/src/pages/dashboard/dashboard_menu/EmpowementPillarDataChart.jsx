...
import EmpowermentChildDataCharts from "./empowerment/EmpowermentChildDataCharts";


const EmpowermentPillarDataChartList = () => {
    ...
    const detailsofEmpowermentPillarGenderChart = async (filters = null) => {
        try {
        
            let requestData = { select_pillar: "empowerment" };
            ...
            if (response.status === 1) {
                const groupedData = response?.data?.total_gender_empowerment_year?.reduce((acc, item) => {
                ...
                const groupedCasteData = response?.data?.total_caste_empowerment_year?.reduce((acc, item) => {
                    if (!acc[item.tfy_year]) {
                ...
                const groupedVulnerabilityData = response?.data?.total_vulnerability_empowerment_year.reduce((acc, item) => {
                    if (!acc[item.tfy_year]) {
                ...
                const groupedSocioEconomicData = response?.data?.total_socio_economic_empowerment_year?.reduce((acc, item) => {
                    if (!acc[item.tfy_year]) {
    ...
    useEffect(() => {
        detailsofEmpowermentPillarGenderChart();
    }, []);
    ...
    const handleSelectChange = (selectedOption, { name }) => {
        ...
        if (name === "year_from") {
            ...
            setYearError(""); // Reset error when selecting new From Year
        } else if (name === "year_to") {
            if (selectedYearFrom && selectedOption.value < selectedYearFrom.value) {
                setYearError("To Year cannot be less than From Year.");
    ...
    const searchFiter = () => {
        ...
        setTimeout(() => {
            detailsofEmpowermentPillarGenderChart(data);
        }, 100);
    };
                                            ...
                                            selectedType?.value == 'yearly' ? (
                                                <>
                                                    ...
                                                    <div className="col-md-3">
                                                        ...
                                                        {yearError && <small className="text-danger">{yearError}</small>} {/* Show error message */}
                                                    </div>
                                                </>
                                            ) : ""
                                        }
...
<div className="row dash-chart-section">
                        ...
                        {/* Child part start */}
                        <div className="row dash-chart-section">
                            <EmpowermentChildDataCharts formData={formData}/>
                        </div>
...
export default EmpowermentPillarDataChartList;