...
import { AddEditKpiOutcome } from "./AddEditKpiOutcome";
import { statusChange } from "../../../Services/Master-service.js";
import { getExcelExportKpiOutcomeList } from "../../../Services/kpi-outcome-service"; // ✅

export const KpiOutcomeMaster = () => {
  const [showModal, setShowModal] = useState(false);
  let [editKpiOutcome, setEditKpiOutcome] = useState(null); // ✅
  let datatable_url = `${import.meta.env.VITE_API_URL}/admin/masters/kpi-outcome/datatable`;
  const [permissions, setPermissions] = useState([]);
  const [exportsLists, setExportsLists] = useState([]);

  // useEffect(() => {
  //     getMyModulePermissionFun('kpi_outcome_list')                     // ✅
  ...
  const initiatedKpiOutcomeDatatable = () => {
    // ✅
    var authToken = getAuthToken();
    var my_url = `${import.meta.env.VITE_API_URL}/admin/masters/kpi-outcome/datatable`;
    ...
    $("#initiated-KPI-datatable").DataTable({
      ...
      columns: [
        ...
        {
          name: "tkpio_is_active", // ✅
          data: "tkpio_is_active",
          searchable: false,
          orderable: false,
            ...
            return `
                            <div class="form-check form-switch d-inline-block"
                                ...
                                <input class="form-check-input toggle-switch"
                                       type="checkbox"
                                       id="toggleSwitch_${row.tkpio_id}"
                                       data-id="${row.tkpio_id}"
                                       ${isChecked}
                                       data-table="t_kpi_outcome_master">
                            </div>
                        `;
          },
        },
  ...
  useEffect(() => {
    if (exportsLists.length > 0) {
      tableToExcel("new-table", "KPI Outcome List"); // ✅
    }
  }, [exportsLists]);
      ...
      <div className="home-content">
        <div className="card pb-3">
          <div className="card-header fw-bold d-flex justify-content-between align-items-center py-2 px-3">
            <h5 className="mb-0 float-left">KPI / Outcome Indicators List</h5>{" "}
            {/* ✅ */}
            <div className="float-right">
              {permissions?.indexOf("add") > -1 && (
                <button
                  ...
                  onClick={() => addFun()}
                >
                  <i className="fa-solid fa-plus"></i> Add KPI Outcome
                </button>
              )}
          ...
          >
            ...
            <Modal.Body>
              <AddEditKpiOutcome
                ...
                editKpiOutcome={editKpiOutcome} // ✅
                initiatedKpiOutcomeDatatable={initiatedKpiOutcomeDatatable} // ✅
          ...
          <tbody>
            {exportsLists.map((data, index) => (
              <tr key={index}>
                ...
                <td>{data?.tkpio_outcome_name ?? "-"}</td> {/* ✅ */}
                <td>{data?.tkpio_desc ?? "-"}</td> {/* ✅ */}
              </tr>
            ))}
          </tbody>
        </table>
