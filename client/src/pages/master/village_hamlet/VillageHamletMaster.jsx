...
import { getExcelExportVillageList, } from '../../../Services/Village-service.js';
...
export const VillageHamletMaster = () => {
    ...
    const handleStatusChange = async (pk, newStatus) => {
        try {
            const payload = {
                table: 't_villages',   // ✅ fixed: was 't_revenue_village', should be 't_villages'
                pk: pk,
    ...
    useEffect(() => {
        if (exportsLists.length > 0) {
            tableToExcel("new-table", "Village List");
        }
    }, [exportsLists]);
            ...
            <div className="home-content">
                <div className="card pb-3">
                    <div className="card-header fw-bold d-flex justify-content-between align-items-center py-2 px-3">
                        <h5 className="mb-0 float-left">Village List</h5>
                ...
                <div className="allModals">
                    ...
                    >
                        <Modal.Header closeButton>
                            <Modal.Title>
                                {editVillage === '' ? 'Add Village' : 'Update Village'}
                            </Modal.Title>
                        </Modal.Header>
                    ...
                    <tbody>
                        {exportsLists.map((data, index) => (
                            <tr key={index}>
                                ...
                                <td>{data?.tvl_village_name ?? "-"}</td>   {/* ✅ added */}
                            </tr>
                        ))}
                    </tbody>
                </table>
