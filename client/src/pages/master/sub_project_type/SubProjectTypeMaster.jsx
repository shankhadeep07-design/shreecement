...
// import { getExcelExportSubProjectTypeList } from '../../../Services/State-service.js';

import { getExcelExportSubProjectTypeList } from '../../../Services/SubProject-type-service';
...
export const SubProjectTypeMaster = () => {
    ...
    const initiatedStateDatatable = () => {
        ...
        var my_url = `${import.meta.env.VITE_API_URL}/admin/masters/sub-project-type/datatable`;
        ...
        $("#initiated-State-datatable").DataTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    data: "tsprj_sub_project_type_name",
                    searchable: true,
                    orderable: true,
                },
                {
                ...
                {
                    name: "tsprj_is_active",
                    data: "tsprj_is_active",
                    searchable: false,
                    orderable: false,
                ...
                <div class="form-check form-switch d-inline-block"
                    ...
                    <input class="form-check-input toggle-switch"
                           type="checkbox"
                           id="toggleSwitch_${row.tsprj_id}"
                           data-id="${row.tsprj_id}"
                          
                           ${isChecked}  data-table="t_sub_project_type"   >
                </div>
            `;
                    }
                },
    ...
    const getAllExportData = () => {
        getExcelExportSubProjectTypeList().then((response) => {
    ...
    useEffect(() => {
        if (exportsLists.length > 0) {
            tableToExcel("new-table", "Sub Project Type List"); // Trigger export once data is set
        }
    }, [exportsLists]);
                        ...
                        <div className="float-right">
                            {
                                ...
                                <button
                                    ...
                                    onClick={() => addFun()}
                                >
                                    <i class="fa-solid fa-plus"></i> Add Sub Project Type
                ...
                >
                    ...
                    <tbody>
                        {exportsLists?.map((data, index) => (
                            <tr key={index}>
                                <td>{index + 1}</td>
                                <td>{data?.tsprj_sub_project_type_name || ""}</td>
                                <td>{data?.tsprj_sub_desc || ""}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
