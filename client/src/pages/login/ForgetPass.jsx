...
import { userLogin, validateProductKey, authEmail } from "../../Services/User-service";
...
export const ForgetPass = ({loginState}) => {
  ...
  const userFormSubmit = async (event) => {
    ...
    try {
      setLoading(true);
      authEmail(loginDetails).then((response) => {
        if (response.status == 1) {
          toast.success(response.message);
          navigate("../change-password", { state: { email: loginDetails.email } });
          setLoading(false);
        } else {
              ...
              <div className="col-lg-6">
                <div class="right-side">
                  <form onSubmit={userFormSubmit}>
                    ...
                    <label for="email"> Registered Username/Email Address</label>
                    <input
                        type="email"
