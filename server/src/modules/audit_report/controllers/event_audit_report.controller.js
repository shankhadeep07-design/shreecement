 ...
 */
module.exports.eventReport = async (req, res, next) => {
  try {
    ...
    const countSql = `
      ...
      FROM audit.event_logged_actions a
    ...
    console.error("❌ eventReport error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};

module.exports.eventReportExcel = async (req, res, next) => {
  try {
    ...
    const sheet = workbook.addWorksheet("Event Audit Report");
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=EventAuditReport.xlsx",
    );

    await workbook.xlsx.write(res);
    res.end();
  } catch (error) {
    console.error("❌ eventReportExcel error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};
