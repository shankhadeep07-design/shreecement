 ...
 */
module.exports.budgetReport = async (req, res, next) => {
  try {
    ...
    const countSql = `
      ...
      FROM audit.budget_logged_actions a
    ...
    console.error("❌ budgetReport error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};

/**
 * Exports budget audit logs to Excel
 */
module.exports.budgetReportExcel = async (req, res, next) => {
  try {
    ...
    const sheet = workbook.addWorksheet("Budget Audit Report");
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=BudgetAuditReport.xlsx",
    );

    await workbook.xlsx.write(res);
    res.end();
  } catch (error) {
    console.error("❌ budgetReportExcel error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};
