...
module.exports.proposalReport = async (req, res, next) => {
  try {
    ...
    // ✅ COUNT QUERY (FIX ADDED)
    const countSql = `
      SELECT COUNT(*) AS total
      FROM audit.proposal_data_logged_actions a
    ...
    console.error("❌ proposalReport error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};


module.exports.proposalReportExcel = async (req, res, next) => {
  try {
    ...
    const sheet = workbook.addWorksheet('Proposal Audit Report');
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=ProposalAuditReport.xlsx');

    await workbook.xlsx.write(res);
    res.end();

  } catch (error) {
    console.error("❌ proposalReportExcel error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};