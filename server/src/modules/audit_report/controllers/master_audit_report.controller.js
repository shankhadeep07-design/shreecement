...
module.exports.masterReport = async (req, res, next) => {
  try {
    ...
    const countSql = `
      ...
      FROM audit.master_data_logged_actions a
    ...
    console.error("❌ masterReport error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};


module.exports.masterReportExcel = async (req, res, next) => {
  try {
    ...
    const sheet = workbook.addWorksheet('Master Audit Report');
  ...
  { header: 'Time',      key: 'action_tstamp_clk',  width: 30 },
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=MasterAuditReport.xlsx');

    await workbook.xlsx.write(res);
    res.end();

  } catch (error) {
    console.error("❌ masterReportExcel error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};
