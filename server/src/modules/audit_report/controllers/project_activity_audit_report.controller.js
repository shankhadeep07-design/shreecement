...
/**
 * Fetches paginated project activity audit logs
 */
module.exports.projectActivityReport = async (req, res, next) => {
  try {
    ...
    const countSql = `
      ...
      FROM audit.project_activity_logged_actions a
    ...
    console.error("❌ projectActivityReport error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};

/**
 * Exports project activity audit logs to Excel
 */
module.exports.projectActivityReportExcel = async (req, res, next) => {
  try {
    ...
    const sheet = workbook.addWorksheet("Project Activity Audit Report");
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=ProjectActivityAuditReport.xlsx",
    );

    await workbook.xlsx.write(res);
    res.end();
  } catch (error) {
    console.error("❌ projectActivityReportExcel error:", error.message);
    next(CustomErrorHandler.internalServerError(error.message));
  }
};
